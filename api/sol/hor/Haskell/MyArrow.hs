...
-- The sandard laws as rewrite rules 
{-# RULES
"1" arr id = id
"2" forall f g . arr ((>>>) f g) = (>>>) (arr f) (arr g)
"3" forall f   . first (arr f) = arr (first f)
"4" forall f g . first (f >>> g) = (first f) >>> (first g)
"5" forall f   . first f >>> arr fst = arr fst >>> f
"6" forall f g . first f >>> arr (id *** g) = arr (id *** g) >>> first f
"7" forall f   . first (first f) >>> arr assoc = arr assoc >>> first f
"a" forall a b c . assoc ((a,b),c) = (a,(b,c))
 #-}
